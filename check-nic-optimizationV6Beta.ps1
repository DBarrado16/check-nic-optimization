...
function Test-Setting {
    ...
    $matchList = $props | Where-Object {
        $_.DisplayName -match $NamePattern -and (-not $ExcludePattern -or $_.DisplayName -notmatch $ExcludePattern)
    }
    if (-not $matchList) {
...
if ($esWifi) {
    ...
    # ExcludePattern MIMO evita mezclar esto con la propiedad separada de
    # ahorro de energia especifica de antenas MIMO (SMPS), que se comprueba
    # aparte justo debajo porque tiene sus propios valores validos.
    ...
    # SMPS (Spatial Multiplexing Power Save) reduce antenas/flujos MIMO para
    # ahorrar bateria. No hay un valor "incorrecto" universal aqui -muchos
    # drivers ni siquiera ofrecen Disabled, solo Static/Dynamic/Automatic-
    # asi que esto es solo informativo, no se marca como fallo.
    $mimoProp = $props | Where-Object { $_.DisplayName -match "MIMO.*Power Save|ahorro de energ.a MIMO" } | Select-Object -First 1
    if ($mimoProp) {
        $cleanMimo = Remove-Diacritics $mimoProp.DisplayValue
        Write-Host "[i] $(Remove-Diacritics $mimoProp.DisplayName) : $cleanMimo (informativo, el valor de fabrica suele ser correcto)" -ForegroundColor Yellow
    }
